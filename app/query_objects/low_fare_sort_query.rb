class LowFareSortQuery
  ...
  def find_all
    @scoped.joins('inner join cities origin on origin.id = low_fares.origin_id').
      ...
      select('low_fares.*, origin.name as origin_name, origin.airport_code as origin_code, destination.name as destination_name, destination.airport_code as destination_code')
  end
end
